...
async function handlePostAuthRoute(
  ...
  const authResult = processPostAuthRequest(bodyWithHost, routeRequest.query, config)
  ...
  // Body parameters take precedence, query parameters will be ignored when body detected
...
export function createRoutesV2(config: Config): Router {
  ...
  router.get(
    '/clear-credentials',
    createRouteHandler((_request: SshRouteRequest) => {
