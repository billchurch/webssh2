...
class SSHConnection extends EventEmitter {
  ...
  private conn: SSH | null = null;
  private stream: ClientChannel | null = null;
  private creds: SSHCredentials | null = null;
  ...
  connect(creds: SSHCredentials): Promise<SSH> {
    debug('connect: %O', maskSensitiveData(creds as unknown as Record<string, unknown>));
    this.creds = creds;
    ...
    return new Promise((resolve, reject) => {
      ...
      try {
        ...
        reject(new SSHConnectionError(`Connection failed: ${(err as Error).message}`));
      }
    });
  }
   ...
   */
  private setupConnectionHandlers(resolve: (value: SSH) => void, reject: (reason: Error) => void): void {
    this.conn!.on('ready', () => {
      debug(`connect: ready: ${this.creds!.host}`);
      resolve(this.conn!);
    });

    this.conn!.on('error', (err: Error & { level?: string; code?: string }) => {
    ...
    (this.conn! as any).on('keyboard-interactive', 
      (name: string, instructions: string, lang: string, prompts: KeyboardInteractivePrompt[], finish: FinishCallback) => {
  ...
  sendPromptsToClient(
    ...
    this.once('keyboard-interactive-response', (responses: string[]) => {
      finish(responses);
    });
  }
   ...
   */
  private getSSHConfig(creds: SSHCredentials, useKey: boolean): Record<string, unknown> {
  ...
  shell(options: TerminalOptions, envVars?: Record<string, string>): Promise<ClientChannel> {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.conn!.shell(ptyOptions, envOptions, (err: Error | undefined, stream: ClientChannel) => {
        if (err) {
          reject(err);
        } else {
          this.stream = stream;
  ...
  resizeTerminal(rows: number, cols: number): void {
    if (this.stream && typeof (this.stream as any).setWindow === 'function') {
      (this.stream as any).setWindow(rows, cols);
    }
  }
