...
function parseArrayValue(value) {
  ...
  if (value.trim().startsWith('[') && value.trim().endsWith(']')) {
    try {
      ...
      debug('Failed to parse JSON array, falling back to comma-separated: %s', err.message)
    }
  }
 ...
 * @returns {Object} Configuration object with values from environment variables
 */
 ...
 * @returns {Object} Map of environment variable names to descriptions
 */
 ...
 * @returns {string} Description of the environment variable
 */
function getEnvVarDescription(envVar, mapping) {
  const descriptions = {
 ...
 * @returns {Object} Available algorithm presets
 */
