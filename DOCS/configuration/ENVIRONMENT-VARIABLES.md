...
| `WEBSSH2_OPTIONS_TRANSPORT` | string or array | `['websocket', 'polling']` | Socket.IO transports in order of preference: comma-separated (`websocket,polling`) or JSON array. Valid values: `websocket`, `polling`. Order is significant. Invalid values logged with warning, defaults to `['websocket', 'polling']`. Requires webssh2_client ≥ 5.3.0. Use for WebSocket-blocking proxies/load balancers |
