...
import { cleanupEnvironmentVariables, storeEnvironmentVariables, restoreEnvironmentVariables } from './test-helpers.mjs'
...
describe('Environment Configuration Tests', () => {
  ...
  test('loadEnvironmentConfig handles simple string values', () => {
    ...
    assert.equal((config as any).ssh.host, 'test.example.com')
    assert.equal((config as any).header.text, 'Test Header')
  })
  ...
  test('loadEnvironmentConfig handles numeric values', () => {
    ...
    assert.equal((config as any).listen.port, 3000)
    assert.equal((config as any).ssh.port, 2222)
    assert.equal((config as any).ssh.readyTimeout, 30000)
  })
  ...
  test('loadEnvironmentConfig supports legacy PORT variable', () => {
  ...
  test('loadEnvironmentConfig WEBSSH2_LISTEN_PORT overrides PORT', () => {
    process.env.PORT = '8080'
    process.env.WEBSSH2_LISTEN_PORT = '9000'

    const config = loadEnvironmentConfig()

    // WEBSSH2_LISTEN_PORT should be processed after PORT
    // Both set the same path, so the last one wins
