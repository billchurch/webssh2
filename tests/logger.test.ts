...
test('createNamespacedDebug creates debug function with correct namespace', () => {
  const debug = createNamespacedDebug('test')
  assert.equal(typeof debug, 'function')
  assert.equal(debug.namespace, 'webssh2:test')
})

test('logError logs error message without error object', () => {
...
test('logError logs error message with error object', () => {
  const consoleMock = mock.method(console, 'error')
